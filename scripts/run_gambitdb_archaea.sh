#!/bin/bash
# run_gambitdb_archaea.sh ~/path/to/workin/dir /path/to/gtdb_file.tsv 1
...
gambitdb-gtdb -i 1 --checkm_completeness 80 --checkm_contamination 10 --include_novel_species --max_contigs 500 -s $BASEDIR/species_taxa.csv -g $BASEDIR/assembly_metadata.csv --include_derived_samples -a $BASEDIR/accessions_to_download.csv $INPUTFILE 
...
ncbi-genome-download -r 3 -A accessions_to_download.csv -F fasta  -o fasta --flat-output -s genbank -p $CORES archaea 
ncbi-genome-download -r 3 -A accessions_to_download.csv -F fasta  -o fasta --flat-output -s refseq -p $CORES archaea 
# some archaea genomes are misclassified as bacteria!
...
gambitdb --small_cluster_ngenomes 1 -v --cpus $CORES -d $BASEDIR/intermediate_files $BASEDIR/fasta $BASEDIR/assembly_metadata.csv $BASEDIR/species_taxa.csv
gambit-create --database_output_filename $BASEDIR/final/database.gdb --signatures_output_filename $BASEDIR/final/database.gs $BASEDIR/intermediate_files/genome_assembly_metadata.csv $BASEDIR/intermediate_files/species_taxon.csv $BASEDIR/intermediate_files/database.gs
