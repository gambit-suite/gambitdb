...
class PairwiseTable:
    ...
    """
        ...
        return 'gambit signatures create -k %s -p %s -o %s -l %s -c %s %s' % (self.kmer, self.kmer_prefix, self.signatures_output_filename, assembly_list_filename, self.cpus, progress_bar)
    ...
    """
        ...
        if self.verbose:
            ''
        return 'gambit dist --qs %s --square -o %s -c %s %s' % (self.signatures_output_filename, self.distance_table_output_filename, self.cpus, progress_bar)
